...
<head>
    ...
    <meta name="description"
        content="{{ __('detail.meta_description', ['name' => $ticket->name, 'desc' => Str::limit(strip_tags($ticket->description), 120)]) }}">
            ...
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-0">
                    ...
                    <li class="breadcrumb-item"><a href="{{ route('ticket.shop') }}"
                            class="text-secondary text-decoration-none">{{ __('detail.breadcrumb_all_tickets') }}</a>
                    </li>
                    <li class="breadcrumb-item active text-info" aria-current="page">{{ $ticket->name }}</li>
                </ol>
            </nav>
            <a href="{{ route('home') }}" class="btn btn-outline-secondary btn-sm"><i class="bi bi-arrow-left"></i>
                {{ __('detail.back_btn') }}</a>
        </div>
            ...
            <div class="col-lg-7 mb-4">
                <div class="custom-scroll-container">
                    ...
                    @if ($ticket->gallery && count($ticket->gallery) > 0)
                        <div id="gameGallery"
                            class="carousel slide mb-4 shadow-sm rounded-4 overflow-hidden border border-light"
                            data-bs-ride="carousel">
                            <div class="carousel-inner">
                                {{-- Lấy ảnh bìa làm ảnh đầu tiên --}}
                                <div class="carousel-item active">
                                    <img src="{{ $ticket->image_url }}" class="d-block w-100"
                                        style="object-fit: cover; height: 450px;" alt="Bìa game">
                                </div>
                                {{-- Các ảnh upload phụ --}}
                                @foreach ($ticket->gallery as $img)
                                    <div class="carousel-item">
                                        <img src="{{ asset('storage/' . $img) }}" class="d-block w-100"
                                            style="object-fit: cover; height: 450px;" alt="Hình ảnh game">
                                    </div>
                                @endforeach
                            </div>
                            <button class="carousel-control-prev" type="button" data-bs-target="#gameGallery"
                                data-bs-slide="prev">
                                <span class="carousel-control-prev-icon" aria-hidden="true"
                                    style="background-color: rgba(0,0,0,0.5); border-radius: 50%; padding: 20px;"></span>
                            </button>
                            <button class="carousel-control-next" type="button" data-bs-target="#gameGallery"
                                data-bs-slide="next">
                    ...
                    <div class="content-box">
                        <h5 class="section-title text-primary"><i class="bi bi-file-text-fill me-2"></i>
                            {{ __('detail.description_title') }}
                        </h5>
                        @php
                        ...
                        <div class="mt-5">
                            <h5 class="section-title text-primary"><i class="bi bi-controller me-2"></i>
                                {{ __('detail.rules_title') }}</h5>
                    ...
                    <div class="text-center mb-4">
                        <div class="d-flex align-items-center justify-content-center gap-2 mb-2">
                            ...
                            <button id="btn-share" class="btn btn-light border rounded-circle shadow-sm"
                                style="width:40px;height:40px;padding:0;flex-shrink:0;" title="Chia sẻ">
                                <i class="bi bi-share-fill text-primary" style="font-size:16px;"></i>
                            </button>
                        </div>

                        {{-- SHARE DROPDOWN --}}
                        <div id="share-panel" class="d-none shadow rounded-4 border bg-white p-3 mx-auto"
                            style="max-width:320px; z-index:100;">
                            <p class="fw-bold text-dark mb-3 text-start small">{{ __('detail.share_title') }}</p>
                            <div class="d-flex justify-content-around mb-2">
                                {{-- Facebook --}}
                                <a href="#" class="share-btn text-center text-decoration-none"
                                ...
                                <a href="#" class="share-btn text-center text-decoration-none"
                                    ...
                                    <div class="rounded-circle d-flex align-items-center justify-content-center mx-auto mb-1"
                                        style="width:50px;height:50px;background:linear-gradient(45deg,#0084FF,#B44FE8);">
                                ...
                                <a href="#" class="share-btn text-center text-decoration-none"
                                    data-platform="zalo">
                                    <div class="rounded-circle d-flex align-items-center justify-content-center mx-auto mb-1"
                                        style="width:50px;height:50px;background:#0068FF;">
                            ...
                            <div class="d-flex align-items-center gap-2">
                                ...
                                <button id="btn-copy-link" class="btn btn-primary btn-sm px-3 fw-bold"
                        ...
                        <div class="d-flex align-items-center justify-content-center gap-3 text-muted">
                            <span class="d-flex align-items-center gap-1">
                                ...
                                {{ $ticket->locations->pluck('name')->join(', ') ?: __('detail.multiple_locations') }}
                            </span>
                        ...
                        <div class="card-body p-4">
                            ...
                            @if ($ticket->ticket_types && count($ticket->ticket_types) > 0)
                                @foreach ($ticket->ticket_types as $index => $type)
                                    <div class="d-flex justify-content-between align-items-center mb-3 p-3 rounded-3 bg-white shadow-sm border border-light hover-scale"
                                        ...
                                        onclick="document.getElementById('type_{{ $index }}').checked = true;">
                                        <div class="form-check m-0">
                                            <input class="form-check-input border-primary" type="radio"
                                                name="selected_ticket_type" id="type_{{ $index }}"
                                                value="{{ $type['name'] }}" {{ $index == 0 ? 'checked' : '' }}>
                                            <label class="form-check-label text-dark ms-2 fw-bold"
                                                for="type_{{ $index }}">
                                                {{ $type['name'] }}
                                            </label>
                                        </div>
                                        <span
                                            class="fw-bold fs-5 text-primary">{{ number_format($type['price']) }}đ</span>
                                    </div>
                                @endforeach
                            @else
                                <div class="text-muted text-center fst-italic mb-3">{{ __('detail.price_updating') }}
                                </div>
                            @endif
                            ...
                            @else
                                <form action="{{ route('booking.form', $ticket->id) }}" method="GET"
                                    ...
                                    <button type="submit"
                                        class="btn w-100 btn-lg fw-bold text-uppercase py-3 shadow-sm rounded-pill"
                                        style="background: linear-gradient(135deg, #2563eb, #0dcaf0); border: none; color: #fff; letter-spacing: 1px;">
                                        {{ __('detail.book_now_btn') }} <i
                ...
                btn.addEventListener('click', function(e) {
                    ...
                    if (platform === 'facebook') {
                        ...
                        url = 'https://www.facebook.com/dialog/send?link=' + pageUrl +
                            '&app_id=291494419107518&redirect_uri=' + pageUrl;
